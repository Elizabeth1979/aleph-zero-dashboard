...
# Custom tools, workflows, data systems, and infrastructure
CUSTOMIZATIONS_DATA=$(cat "$DASH/customizations.json" 2>/dev/null || echo "[]")
...
files = [
  ...
  ('manifest.webmanifest', 'Web App Manifest', 'Makes the dashboard installable as an Android app.'),
  ('service-worker.js', 'Dashboard Service Worker', 'Keeps dashboard navigation available when the network drops.'),
  ('pwa.js', 'PWA Registration', 'Registers the dashboard service worker in the browser.'),
  ...
  ('customizations.json', 'Custom Setup Inventory', 'Elli-specific tools, workflows, data systems, and infrastructure.'),
